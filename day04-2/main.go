...
func validatePassphrase(row string) int {
	...
	if len(words) == len(strings.Split(row, " ")) {
		return 1
	}
	return 0
}
...
func main() {
	...
	/*	url := "http://adventofcode.com/2017/day/4/input"
		...
		strData := string(data)
	*/
	strData := `oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
qrwu mgnw hvflf ytspp mco ikvbqg fflvh wts cbb`

	//	strData = "abcde xyz ecdab"
	for _, str := range strings.Split(strData, "\n") {
		sum += validatePassphrase(str)
	}
